...
const slides = [
  { title: 'Sách hay mùa hè này', subtitle: 'Khám phá những cuốn sách hay nhất cho mùa hè này', image: '/images/hero/summer.png', cta: 'Mua ngay', link: '/products' },
  { title: 'Sách mới: Những câu chuyện mới', subtitle: 'Khám phá những kiệt tác mới nhất từ khắp nơi trên thế giới.', image: '/images/hero/new-arrivals.png', cta: 'Xem sách mới', link: '/products?sort=createdAt&order=DESC' },
  { title: 'Quà tặng tri thức', subtitle: 'Gửi tặng những cuốn sách hay kèm lời chúc ý nghĩa đến những người thân yêu.', image: '/images/hero/gift-box.png', cta: 'Chọn quà ngay', link: '/products' },
];

const containerVariants = { hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1, delayChildren: 0.2 } } };
const itemVariants = { hidden: { y: 20, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.5 } } };

function ProductCard({ book, index }: { book: Product; index?: number }) {
  return (
    <motion.div
    ...
    >
      ...
      {index !== undefined && (
        <div className="absolute top-0 right-0 w-16 h-16 overflow-hidden z-20 pointer-events-none">
          <div className={`absolute top-2 right-[-20px] rotate-45 w-20 py-0.5 text-center text-[10px] font-black uppercase tracking-tighter shadow-sm ${index === 0 ? 'bg-yellow-400 text-yellow-900' : index === 1 ? 'bg-gray-300 text-gray-800' : index === 2 ? 'bg-amber-600 text-amber-50' : 'bg-red-500 text-white'}`}>
            TOP {index + 1}
          </div>
        </div>
      )}
      ...
      <div className="mt-2.5 flex-1">
        <h3 className="text-sm font-semibold text-gray-900 leading-tight line-clamp-2 group-hover:text-red-600 transition-colors h-9">{book.title}</h3>
        ...
        <p className="mt-1 text-base font-bold text-red-600">{formatVND(Number(book.price))}</p>
      </div>
    </motion.div>
  );
}
...
export default function Home() {
  ...
  const { data: bestSellersData, isLoading: loadingBest } = useProducts({ sort: 'soldCount', order: 'DESC', limit: 5 });
  const { data: exclusivesData, isLoading: loadingExcl } = useProducts({ special: true, limit: 5 });

  const bestSellers = bestSellersData?.data ?? [];
  const exclusives = exclusivesData?.data ?? [];
  const loading = loadingBest || loadingExcl;
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            {loading ? (
              <div className="mt-8 grid grid-cols-2 gap-4 lg:grid-cols-5">
                {[...Array(5)].map((_, i) => <div key={i} className="h-96 rounded-2xl bg-white shadow-sm border border-gray-100 animate-pulse" />)}
              </div>
            ) : bestSellers.length > 0 ? (
              <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-100px' }}
                className="mt-8 grid grid-cols-2 gap-4 sm:gap-x-6 lg:grid-cols-5 xl:gap-x-8">
                {bestSellers.map((book, index) => <ProductCard key={book.id} book={book} index={index} />)}
              </motion.div>
            ) : (
            ...
            {!loading && exclusives.length > 0 && (
              <div className="mt-20">
                <div className="flex items-center justify-between">
                  <h2 className="text-3xl font-bold tracking-tight text-gray-900 bg-clip-text text-transparent bg-gradient-to-r from-amber-500 to-orange-600">ĐỘC QUYỀN TẠI DreamBook</h2>
                  <Link href="/products?special=true" className="hidden text-sm font-semibold text-red-600 hover:text-red-500 sm:flex sm:items-center">Xem tất cả sản phẩm<ArrowRight className="ml-1 h-4 w-4" /></Link>
                </div>
                <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-100px' }}
                  className="mt-8 grid grid-cols-2 gap-4 sm:gap-x-6 lg:grid-cols-5 xl:gap-x-8">
                  {exclusives.map((book) => <ProductCard key={book.id} book={book} />)}
                </motion.div>
                <div className="mt-8 sm:hidden">
                  <Link href="/products?special=true" className="block text-sm font-semibold text-red-600 hover:text-red-500">Xem tất cả sản phẩm<ArrowRight className="ml-1 inline h-4 w-4" /></Link>
                </div>
              </div>
            )}
